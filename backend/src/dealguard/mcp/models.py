...
class SearchEdiktsdateiInput(BaseModel):
    ...
    bundesland: Optional[Bundesland] = Field(
        default=None,
        description="Filter by Austrian federal state (optional)"
    )
    limit: int = Field(
        default=10,
...
class CheckInsolvencyInput(BaseModel):
    ...
    company_name: str = Field(
        ...,
        description="Exact or partial company name to check",
        min_length=2,
        max_length=200
    )
...
class CheckSanctionsInput(BaseModel):
    ...
    aliases: Optional[List[str]] = Field(
        default=None,
        description="Alternative names to also check",
        max_length=5
    )
    response_format: ResponseFormat = Field(
        default=ResponseFormat.MARKDOWN,
...
class CheckPEPInput(BaseModel):
    ...
    country: str = Field(
        default="AT",
        description="Country code for context",
        min_length=2,
        max_length=2
    )
    response_format: ResponseFormat = Field(
...
class ComprehensiveComplianceInput(BaseModel):
    ...
    entity_type: EntityType = Field(
        ...
        description="Type of entity: 'company' or 'person'"
    )
    country: str = Field(
        default="AT",
        description="Country code for context",
...
class GetDeadlinesInput(BaseModel):
    ...
    days_ahead: int = Field(
        default=30,
        description="How many days ahead to look for deadlines",
        ge=1,
        le=365
    )
    include_overdue: bool = Field(
        default=True,
        description="Include overdue deadlines in results"
    )
    limit: int = Field(
        default=20,
        description="Maximum number of results",
